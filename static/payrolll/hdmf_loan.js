...
const insert_hdmf_loan = async () => {
    ...
    try {
        const response = await fetch(`/api-insert-hdmf-loan/`, {
            method: "POST",
        ...
        if (response.ok) {
            ...
            window.location.assign("/insert-hdmf-loan/");
...
const btn_save_hdmf_loan_deduction = document.querySelector('#btn_save_hdmf_loan_deduction');
btn_save_hdmf_loan_deduction.addEventListener("click", insert_hdmf_loan);
    ...
    function populateTable() {
        $.ajax({
            ...
            success: function(response) {
                if (response && response.data && response.data.getHdmfLoanDeductions && Array.isArray(response.data.getHdmfLoanDeductions)) {
                    console.log('Data received from server:', response.data.getHdmfLoanDeductions);
                    var tableBody = $('#table_sss_loan tbody');
                    tableBody.empty(); // Clear table body

                    // Populate table rows with data
                    response.data.getHdmfLoanDeductions.forEach(function(item) {
...
$(document).ready(function() {
    ...
    $('#btn_update_hdmf_loan').click(function() {
        $('#update_hdmf_loan_modal').modal('show');
    });
});
...
$(document).ready(function() {
    ...
    function fetchData() {
        ...
        $.ajax({
            ...
            data: JSON.stringify({
                query: `
                    query getHdmfLoanById($searchTerm: String!) {
                        getHdmfLoanById(searchTerm: $searchTerm) {
                            amountDeduction
                        }
                    }
                `,
            ...
            success: function(response) {
                ...
                if (response.data && response.data.getHdmfLoanById) {
                    var amountDeductionValue = response.data.getHdmfLoanById[0].amountDeduction; // Access the first element of the array
    ...
    function updateHDMFDeduction() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#update_hdmf_loan_modal').modal('hide'); // Close the modal
                window.location.href = '/insert-hdmf-loan/';
            },
            error: function(error) {
